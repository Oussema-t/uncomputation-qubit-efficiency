...
def von_neumann_entropy(rho: np.ndarray) -> float:
    ...
    # Eigenvalues are probabilities, so they belong in [0, 1]. Round-off can put
    # a pure state's eigenvalue at 1 + 1e-16, whose log2 is *positive*, which
...
def emit_js_fixture(
    path: str, n_data: int, n_steps: int, seed: int
) -> Dict[str, object]:
    """Write a cross-language fixture for the browser simulation.

    The browser build re-implements the same physics in JavaScript. Two
    independent implementations agreeing is much stronger evidence than either
    one alone, but only if they are fed the *same* problem instance -- the two
    languages' RNGs differ, so the steps have to be handed over explicitly.

    Emits a plain ``var FIXTURE = {...};`` assignment rather than JSON so that
    ``simulation/verify_core.js`` can pull it in with ``load()`` in a bare JS
    engine, without needing a file-reading API.

    Returns:
        The fixture contents, so callers can inspect them without re-reading.
    """
    steps = build_steps(n_data, n_steps, seed)
    specs = make_input_specs(n_data, seed)

    wires_b = data_wire_labels(n_data) + ancilla_wire_labels(ANCILLAS_PER_STEP)
    diagnostics = ancilla_diagnostics(steps, n_data, specs["uniform"], n_steps - 1)

    state_b = simulate(uncomputed_circuit(steps, n_data, specs["uniform"]), wires_b)
    rho_b = partial_trace(state_b, len(wires_b), list(range(n_data)))
    psi_ideal, rho_naive_uniform = structured_states(steps, n_data, specs["uniform"])
    _, rho_naive_basis = structured_states(steps, n_data, specs["basis"])
    psi_basis, _ = structured_states(steps, n_data, specs["basis"])
    ...
    fixture: Dict[str, object] = {
        ...
        "n_steps": n_steps,
        "seed": seed,
        "basis_bits": [int(b) for b in specs["basis"].bits],
        "steps": [
            {
                "qubits": list(step.qubits),
                "negations": [bool(n) for n in step.negations],
                "theta": step.theta,
            }
            for step in steps
        ],
        "expected": {
            ...
            "naive_ancillas": naive_ancillas,
            "uncomputed_total_qubits": unc_total,
            "entropy_before": diagnostics["entropy_before"],
            "entropy_after": diagnostics["entropy_after"],
            "purity_before": diagnostics["purity_before"],
            "purity_after": diagnostics["purity_after"],
            "fidelity_uncomputed_vs_ideal": fidelity(
                rho_b, density_matrix(psi_ideal)
            ),
            "fidelity_naive_uniform": fidelity(
                rho_naive_uniform, density_matrix(psi_ideal)
            ),
            "trace_distance_naive_uniform": trace_distance(
                rho_naive_uniform, density_matrix(psi_ideal)
            ),
            "fidelity_naive_basis": fidelity(
                rho_naive_basis, density_matrix(psi_basis)
            ),
        },
    }
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "write a cross-language fixture (problem instance + reference "
            "metrics) for simulation/verify_core.js"
        ),
    )
    parser.add_argument(
        "--fixture-steps",
        type=int,
        default=5,
        help="N to use for --emit-js-fixture",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.emit_js_fixture:
        try:
            emit_js_fixture(
                args.emit_js_fixture,
                n_data=args.n_data,
                n_steps=args.fixture_steps,
                seed=args.seed,
            )
        except (ValueError, OSError) as exc:
            LOGGER.error("could not write the JS fixture: %s", exc)
            return 1
